...
// Modified from xAI's bridge runtime for Pi data roots and private state permissions (2026).
...
const PLUGIN_DATA_ENV = "GROK_PI_DATA";
const FALLBACK_STATE_ROOT_DIR = path.join(os.tmpdir(), "grok-pi-runs");
...
export function normalizeJobId(value, label = "job id") {
  const normalized = String(value ?? "").trim();
  if (
    !JOB_ID_PATTERN.test(normalized) ||
    normalized === "." ||
    normalized === ".."
  ) {
...
export function claimJobTerminal(cwd, jobId, nextStatus, patch = {}) {
  ...
  return withStateLock(cwd, () => {
    ...
    if (existing.cancelRequested && nextStatus !== "cancelled" && !isTerminalJobStatus(currentStatus)) {
      return {
        claimed: false,
    ...
    if (isTerminalJobStatus(currentStatus)) {
      ...
      if (currentStatus === "cancelled" && nextStatus === "cancelled") {
        ...
        upsertJobInState(state, {
          ...
          pid: null,
          agentPid: null,
          agentIdentity: merged.agentIdentity ?? existing.agentIdentity ?? null,
          bridgeIdentity: merged.bridgeIdentity ?? existing.bridgeIdentity ?? null,
    ...
    upsertJobInState(state, {
      ...
      agentIdentity: nextJob.agentIdentity ?? existing.agentIdentity ?? null,
      bridgeIdentity: nextJob.bridgeIdentity ?? existing.bridgeIdentity ?? null,
...
export function patchJobIfActive(cwd, jobId, patch = {}) {
  return withStateLock(cwd, () => {
    ...
    upsertJobInState(state, {
      ...
      agentIdentity: nextJob.agentIdentity ?? null,
      bridgeIdentity: nextJob.bridgeIdentity ?? null,
...
function pruneJobs(jobs) {
  ...
  const terminalHistory = sorted
    .filter((job) => job.status !== "queued" && job.status !== "running")
    .slice(0, MAX_JOBS);
  const retainedIds = new Set([...active, ...terminalHistory].map((job) => job.id));
...
export function generateJobId(prefix = "job") {
  const safePrefix = normalizeJobId(prefix, "job id prefix");
...
export function resolveJobLogFile(cwd, jobId) {
  ensureStateDir(cwd);
  return path.join(resolveJobsDir(cwd), `${normalizeJobId(jobId)}.log`);
}

export function resolveJobFile(cwd, jobId) {
  ensureStateDir(cwd);
  return path.join(resolveJobsDir(cwd), `${normalizeJobId(jobId)}.json`);
}
