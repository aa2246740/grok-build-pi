...
// Modified from xAI's bridge runtime for Pi host status reporting (2026).

import { getConfig, listJobs, normalizeJobId, readJobFile, resolveJobFile } from "./state.mjs";
...
export function getSessionRuntimeStatus() {
  return {
    ...
    detail: "Runs are tracked by the Grok Build ↔ Pi bridge (PID + log files). There is no shared app-server broker.",
    endpoint: null
  };
}
...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  const safeReference = normalizeJobId(reference, "run reference");

  const exact = filtered.find((job) => job.id === safeReference);
  if (exact) {
    return exact;
  }

  const prefixMatches = filtered.filter((job) => job.id.startsWith(safeReference));
...
export function buildStatusSnapshot(cwd, options = {}) {
  ...
  const storedJobs = listJobs(workspaceRoot);
  const jobs = sortJobsNewestFirst(options.all ? storedJobs : filterJobsForCurrentSession(storedJobs, options));
